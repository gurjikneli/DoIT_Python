...
# დაწერეთ პროგრამა, რომელიც ტერმინალში მომხმარებელს გამოუტანს სტუდენტების აიდის (id) სიას, მომხარებელი აირჩევს სტუდენტის
# აიდის, მიღებული აიდისთვის დაბეჭდავს სტუდენტის მონაცემებს. მონაცემებში უნდა დაიბეჭდოს
...
lst = [i['id'] for i in my_dict["students"]]

print(f'studentebis ID: ', end=" ")
...
for i in my_dict["students"]:
    if i['id'] == st_id:
        print(f'\nstudent infomration: \nID: {i["id"]}, Name: {i["name"]}, Age: {i["age"]}')
        for j in my_dict["subjects"]:
            print(f'subject: {j['name']}, grade: {j["grades"][str(st_id)]}')
